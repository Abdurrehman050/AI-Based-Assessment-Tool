...
export default function TeacherDashboard() {
  ...
  const totalSubmissions = exams.reduce(
    (acc, e) => acc + (e.attemptedCount || 0),
    0,
  );
  const gradedSubmissions = exams.reduce(
    (acc, e) => acc + (e.gradedCount || 0),
    0,
  );
  const avgScore =
    totalSubmissions > 0
      ? (
          exams.reduce(
            (acc, e) => acc + (e.averageScore || 0) * (e.attemptedCount || 0),
            0,
          ) / totalSubmissions
        ).toFixed(2)
      : 0;
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-6 md:px-12">
      ...
      <div className="bg-white shadow-md rounded-2xl p-8 mb-6 flex flex-col md:flex-row items-center md:justify-between space-y-4 md:space-y-0">
        <div className="flex items-center space-x-6">
          <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center text-2xl font-bold text-primary">
        ...
        <button
          ...
          className="px-6 py-3 rounded-full bg-primary text-white font-semibold hover:bg-primary/80 transition shadow-lg"
        >
          Manage Exams
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div className="bg-white rounded-2xl shadow-md p-6 flex flex-col items-center hover:shadow-xl transition">
          ...
          <p className="text-3xl font-bold text-primary mt-2">{totalExams}</p>
        </div>
        <div className="bg-white rounded-2xl shadow-md p-6 flex flex-col items-center hover:shadow-xl transition">
        ...
        <div className="bg-white rounded-2xl shadow-md p-6 flex flex-col items-center hover:shadow-xl transition">
          ...
          <p className="text-3xl font-bold text-primary mt-2">{avgScore}</p>
        </div>
      </div>
            ...
            >
              ...
              <div className="mt-4 flex gap-2 flex-wrap">
                <button
                  ...
                  className="px-4 py-2 rounded-full bg-primary text-white font-medium hover:bg-primary/80 transition flex-1"
                >
                  Preview
                </button>
                <button
