...
export default function CandidateDashboard() {
  ...
  // Fetch profile only if user is not loaded yet
  useEffect(() => {
  if (user) {
    setAttemptedExams(user.info.attemptedExams || []);
    setLoading(false);
    return;
  }
      ...
      <div className="bg-white shadow-md rounded-2xl p-8 mb-6 flex flex-col md:flex-row items-center md:justify-between space-y-4 md:space-y-0">
        <div className="flex items-center space-x-6">
          ...
          <div>
            ...
            <p className="text-gray-500 mt-1">Institution: {user.info.institution || "N/A"}</p>
          </div>
        </div>
        <button
onClick={() => navigate("/candidate/enter-exam")}
          className="px-6 py-3 rounded-full bg-blue-600 text-white font-semibold hover:bg-blue-700 transition shadow-lg"
        >
          Enter Exam Key
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div className="bg-white rounded-2xl shadow-md p-6 flex flex-col items-center hover:shadow-xl transition">
          ...
          <p className="text-3xl font-bold text-blue-600 mt-2">{totalExams}</p>
        </div>
        <div className="bg-white rounded-2xl shadow-md p-6 flex flex-col items-center hover:shadow-xl transition">
        ...
        <div className="bg-white rounded-2xl shadow-md p-6 flex flex-col items-center hover:shadow-xl transition">
          ...
          <p className="text-3xl font-bold text-blue-600 mt-2">{lastExam}</p>
        </div>
      </div>

      {/* Exams Section */}
      <h2 className="text-2xl font-semibold text-gray-700 mb-6">Attempted Exams</h2>
      {user.info.attemptedExams?.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {user.info.attemptedExams.map((sub) => (
            <div
              key={sub._id}
              className="bg-white rounded-2xl shadow-md p-6 hover:shadow-xl transition cursor-pointer flex flex-col justify-between"
            >
              <div>
                <h3 className="text-lg font-semibold text-gray-800">{sub.exam?.title || "Untitled Exam"}</h3>
                <p className="text-sm text-gray-500 mt-1">Exam Key: {sub.exam?.examKey || "-"}</p>
                {sub.score !== undefined && (
                  <p className="text-sm text-green-600 mt-2 font-medium">Score: {sub.score}</p>
                )}
              </div>
              <button
        ...
        <p className="text-gray-500 text-center mt-6">You have not attempted any exams yet.</p>
      )}
    </div>
  );
}
